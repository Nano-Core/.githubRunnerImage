...
# sqlcmd (Go)
RUN GOSQLCMD_VERSION=$(curl -s https://api.github.com/repos/microsoft/go-sqlcmd/releases/latest | grep '"tag_name"' | cut -d '"' -f4) && \
    curl -L "https://github.com/microsoft/go-sqlcmd/releases/download/${GOSQLCMD_VERSION}/sqlcmd-${GOSQLCMD_VERSION}-linux-amd64.tar.bz2" -o sqlcmd.tar.bz2 && \
